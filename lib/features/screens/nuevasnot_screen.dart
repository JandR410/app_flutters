...
class NuevasNotScreen extends StatelessWidget {
  final idUser;
  final nameId;
  final id;
  final idCollection;
  final ocTabBar;
  const NuevasNotScreen({
    Key? key,
    ...
    return Scaffold(
        body: StreamBuilder(
            stream: FirebaseFirestore.instance
                .collection('notifications')
                .where("employee_id", isEqualTo: idUser)
                .where("status",
                    isEqualTo: "SEND") // Replace with your subcollection
                .snapshots(),
            builder:
              ...
              return (snapshot.connectionState == ConnectionState.waiting)
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                                        ...
                                        child: StreamBuilder(
                                          ...
                                          builder: (BuildContext context,
                                              AsyncSnapshot<QuerySnapshot>
                                                  snapshot) {
                                            return CardNotNuevasFormScreen(
                                              mensaje: message,
                                              title: title,
                                              status: status,
                                            );
                        ...
                        if (data['status'].toString().startsWith(name!)) {
                          final idCollection = collectionId;
                          final message = data['message'];
                          final title = data['title'];
                          final status = data['purchase_order_status'];
